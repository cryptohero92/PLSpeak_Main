...
contract CryptoMarketFactoryV2 is AbstractMarketFactoryV2, CalculateLinesToBPoolOdds, UsingFetch {
    ...
    bytes32 public queryId = keccak256(abi.encode("SpotPrice", abi.encode("eth", "usd")));
    ...
    function addCoin(
        ...
        Coin memory _coin = makeCoin(_name, _imprecision);
    ...
    // Utility function to truncate the price based on imprecision
    function _truncatePrice(uint256 _price, uint8 _imprecision) internal pure returns (uint256) {
        return _price / (10 ** _imprecision) * (10 ** _imprecision);
    }
    ...
    function getPrice(Coin memory _coin,  
        uint80 _roundId,uint256 _resolutionTime) internal returns (uint256 _fullPrice, uint256 _truncatedPrice) {
        (bytes memory _value, uint256 _timestampRetrieved) = getDataBefore(queryId, block.timestamp - 15 minutes);
        require(_timestampRetrieved > 0, "No data was found");
        require(block.timestamp - _timestampRetrieved < 24 hours, "Data is too old");
        // Convert bytes to uint256
        _fullPrice = _sliceUint(_value);

        // Truncate the price based on the coin's imprecision
        _truncatedPrice = _truncatePrice(_fullPrice, _coin.imprecision);
    ...
    function makeCoin(
        ...
        _coin = Coin(_name, 0, _imprecision, _currentMarkets);
    }
